...
#Preparations
taxa <- c('Class', 'Order', 'Family', 'Genus')
info <- array(0, dim = c(1,8))

#Data input
path <- 'D:/PythonProject/Density_dependency_S_map/DD_S-map_2/outputs/taxa'
  ...
  for (WWTP in WWTPs) {
    ...
    for (direction in c('increase','decrease')) {
      ...
      for (i in 1:length(fit_result_files)) {
        ...
        info <- rbind(info, cbind(file_info[1], taxa[j], fit_result$X, 
...
ggplot(info, aes(x=Theta, y=`RMSE/STD`))+
  ...
  scale_x_discrete(limits=c('Class', 'Order', 'Family', 'Genus'))+
  ylim(0,2)+
